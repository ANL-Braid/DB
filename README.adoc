...
3. If you want the funcx hosted braid db to store files in a different location, edit `src/braid_db/funcx/funcx_main.py` and change the value of `DB_FILE` in the function `funcx_add_record`. If not editted the funcx-based operations will store their entries in the file `~/funcx-braid.db`.
